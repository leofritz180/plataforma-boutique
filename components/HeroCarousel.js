...
export default function HeroCarousel() {
  ...
  function handleTouchStart(e) { touchStartRef.current = e.touches[0].clientX }
  function handleTouchMove(e) { touchEndRef.current = e.touches[0].clientX }
    ...
    <section className="relative overflow-hidden">
      ...
      >
        ...
        <div className="container-custom relative z-10 h-full flex items-center">
          <div key={slide.id} className="w-full text-center animate-fade-in-up">
            <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm rounded-full px-4 py-1.5 mb-5 shadow-sm">
              <SlideIcon className={`w-4 h-4 ${slide.iconColor}`} strokeWidth={2} />
            ...
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl text-brand-dark leading-[1.1] mb-3">
              {slide.title}{' '}
              <span className={`${slide.accentColor} font-bold italic`}>{slide.highlight}</span>
            </h2>

            <p className="text-sm md:text-base text-gray-500 mb-7 max-w-md mx-auto">{slide.subtitle}</p>

            <div className="flex gap-3 justify-center">
              <Link href={slide.href} className="btn-primary text-center px-8 py-3.5">{slide.cta}</Link>
              <Link href="/produtos" className="btn-outline text-center px-8 py-3.5">Ver coleção</Link>
            </div>
          </div>
        </div>
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={`transition-all duration-300 rounded-full ${
                idx === current ? 'w-7 h-2.5 bg-brand-dark' : 'w-2.5 h-2.5 bg-brand-dark/20 hover:bg-brand-dark/40'
              }`}
